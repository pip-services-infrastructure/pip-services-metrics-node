...
export class MetricsMemoryPersistence extends IdentifiableMemoryPersistence<MetricRecordV1, string> implements IReconfigurable, IMetricsPersistence
    {
        ...
        private composeFilter(filter:FilterParams):any
        {
            ...
            if (_.isString(names))
                names = names.split(','); //W! todo need to test
            if (!_.isArray(names))
                names = null;
            //let names = _.toStringArray(filter.getAsNullableString("names"));
            ...
            return (item) => {
                ...
                if (names != null && _.indexOf(names, item.name) < 0)
                    return false;
                if (timeHorizon != null && item.timeHorizon != timeHorizon)
                    return false;
                if (fromRange != null && item.Range < fromRange)
                    return false;
                if (toRange != null && item.Range > toRange)
                    return false;
        ...
        //public set(correlationId:string, item:MetricRecordV1, callback?:(err:any, item:MetricRecordV1)=>void):void;
                ...
                if (timeHorizon <= maxTimeHorizon) {
                    ...
                    if (index < 0)
                    {
                        item = new MetricRecordV1();
                            ...
                            item.val = new Map<string, MetricRecordValueV1>()
